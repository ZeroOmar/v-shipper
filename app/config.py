...
class ConfigManager:
    ...
    def validate_auth(self, username: str, password: str) -> bool:
        ...
        try:
            # Decode password from config (base64 encoded)
            config_password = base64.b64decode(
                self.config.web_ui.admin_password
            ).decode('utf-8')
            
            return (username == self.config.web_ui.admin_user and 
